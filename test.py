...
if __name__ == "__main__":
    ...
    parser.add_argument('--cpu', action='store_false', dest='use_cuda', default=True,
    ...
    model = RoomTypeClassifier(use_cuda=True, use_batch_processing=args.use_batch_processing)

    if not args.input:
        args.input = '/home/proptx/dataset/CLIP'

    if os.path.isdir(args.input):
        img_classes = [os.path.join(args.input, f) for f in os.listdir(args.input) if os.path.isdir(os.path.join(args.input, f))]
    else:
    ...
    for top_k in [x for x in range(args.get_topk_results+1) if x != 0]:
        ...
        if  args.use_batch_processing:
            predicted_classes, true_labels, labels_mapping = model.batch_predict(dir_path=args.input, top_k=top_k, batch_size=BATCH_SIZE)
            for class_labels in predicted_classes:
                predicted_from_clip.append(class_labels)   

        short_to_full_name = {"Balcony":"Balcony", "Bar":"Bar", "Basement":"Basement", "Bathroom":"Bathroom",
                                "Bedroom":"Bedroom", "Children":"Children Room", "Cinema":"Cinema Room",
                                "Closet":"Closet", "Dining":"Dining","Empty":"Empty Room", "Game":"Game Room", 
                                "Garage":"Garage", "Garden":"Garden", "Gym":"Gym", "Kitchen":"Kitchen",
                                "Laundry":"Laundry", "Library":"Library", "Living":"Living Room", "Loft":"Loft",
                                "Office":"Office", "Pool":"Swimming Pool", "Storage":"Storage Room",
                                "Terrace":"Terrace", "Walk":"Walk in Closet", "Wine":"Wine Room",
                                "Informal Dinning Room": "Informal Dinning Room"}
        
        if args.use_batch_processing:
            updated_labels_mapping = {k.replace('_', ' ').title():v for k, v in labels_mapping.items()}
            updated_labels_mapping = {k.replace('/', ' ').title():v for k, v in labels_mapping.items()}
            # for k, v in updated_labels_mapping.items():

            if 'Children Room' in updated_labels_mapping:
                updated_labels_mapping["Children's Room"] = updated_labels_mapping.pop('Children Room') 
            if 'Dining' in updated_labels_mapping:
                updated_labels_mapping['Dining Room'] = updated_labels_mapping.pop('Dining')
            if 'Pool' in updated_labels_mapping:
                updated_labels_mapping["Garden/Yard"] = updated_labels_mapping.pop('Pool') 
            if 'Library Building' in updated_labels_mapping:
                updated_labels_mapping["Library/Study"] = updated_labels_mapping.pop('Library Building') 
            if 'Wineroom' in updated_labels_mapping:
                updated_labels_mapping['Wine Cellar'] = updated_labels_mapping.pop('Wineroom')
        ...
        # Precision
        precision = precision_score(true_labels, pruned_predicted_labels, average='weighted', zero_division=0)
        # Recall
        recall = recall_score(true_labels, pruned_predicted_labels,  average='weighted', zero_division=0)
        # F1_score
        ...
        precised_class_accuracies = ['%.3f' % accurcy for accurcy in confusion_mat.diagonal()]
        # Combine label and its corresponding accuracy
        class_wise_accuracy = list(zip(list(updated_labels_mapping.keys()), precised_class_accuracies))
        ...
        if top_k == 1:
            headers = ['Top_K'] + list(updated_labels_mapping.keys())+ ['combined accuracy', 'precision', 'recall', 'f1']
            writer.writerow(headers)
        cumulative_results = [accuracy, precision, recall, f1_Score]
        result_row = [top_k]+[accuracy for _, accuracy in class_wise_accuracy]+['%.3f' % result for result in cumulative_results]
